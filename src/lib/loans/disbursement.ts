  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.transaction.create({
      data: {
        ...
        status: "SUCCESSFUL",
        description: `Loan disbursed to M-Pesa${mpesaReceipt ? ` (Receipt ${mpesaReceipt})` : " (simulated)"}`,
        relatedType: "Loan",
        relatedId: loan.id,
      },
    });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.transaction.create({
      data: {
        ...
        amount: new Prisma.Decimal(amount),
        status: "SUCCESSFUL",
        description: `Loan repayment${mpesaReceipt ? ` (Receipt ${mpesaReceipt})` : " (simulated)"}`,
        relatedType: "Loan",
        relatedId: loan.id,
      },
    });
