...
import { settleWithdrawalManually } from "@/lib/savings/service";
...
// POST /api/admin/withdrawals/[id]/decision — approving settles the payout
// immediately (balance debited, request COMPLETED). Withdrawals do NOT depend
// on the Daraja B2C API: the admin sends the money to the customer's M-Pesa
// out-of-band. Reject simply closes the request without moving money.
export default withApi(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  await settleWithdrawalManually(request.id, admin.id, amount, body.note ?? null);
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entityType: "WithdrawalRequest",
      entityId: request.id,
      details: JSON.stringify({ settledManually: true, note: body.note ?? null }),
    },
  });

  ok(res, { id: request.id, status: "COMPLETED" });
});
